...
from typing import Dict, List, Any, Optional, Tuple, Union, Callable, Generator, Iterable, Set, TypeVar, Generic, cast
...
CONFIG = {
    ...
    "vector_db_enabled": False,  # Enable vector database for semantic search
    "vector_db_url": "http://localhost:8080",  # Vector database server URL
    "speech_api_url": "http://localhost:5000",  # Speech API server URL
    "speech_api_enabled": False,  # Enable speech recognition and synthesis
    ...
    "vision_model": "gemini-2.5-pro-vision",  # Vision model for image analysis
    "fallback_model": "gemini-1.5-pro",  # Fallback model if primary fails
    "use_streaming": True,  # Use streaming for responses
    "enable_model_fallback": True,  # Enable fallback to alternative model if primary fails
    "enable_multimodal": True,  # Enable multimodal capabilities (vision, audio)
    ...
    "sandbox_mode": False,  # Run commands in a sandboxed environment
    "auto_retry_failed_actions": True,  # Automatically retry failed actions
    "max_retries": 3,  # Maximum number of retries for failed actions
    "retry_delay": 2,  # Delay between retries in seconds
    "enable_action_validation": True,  # Validate actions before execution
    ...
    "memory_pruning_threshold": 1000,  # Number of items before memory pruning
    "enable_request_batching": True,  # Batch multiple requests to reduce API calls
    "batch_size": 5,  # Maximum number of requests in a batch
    "enable_compression": True,  # Compress data for storage and transmission
    ...
    "enable_rich_output": True,  # Enable rich text output with formatting
    "enable_syntax_highlighting": True,  # Enable syntax highlighting for code
    "enable_auto_suggestions": True,  # Enable auto-suggestions based on history
    "enable_interactive_mode": True,  # Enable interactive mode with real-time feedback
    "theme": "dark",  # UI theme (dark, light, auto)
    "enable_notifications": True,  # Enable desktop notifications for long-running tasks
    ...
    "enable_pdf_processing": False,  # Enable PDF processing capabilities
    "enable_database_operations": False,  # Enable database operations (disabled by default for security)
    "enable_api_integration": True,  # Enable integration with external APIs
    "enable_git_operations": True,  # Enable Git operations
    "enable_docker_operations": False,  # Enable Docker operations
    "enable_cloud_integration": False,  # Enable cloud service integration
    "enable_semantic_search": False,  # Enable semantic search in files and memory
    "enable_image_analysis": True,  # Enable image analysis using vision model
    "enable_data_visualization": True,  # Enable data visualization capabilities
    "enable_voice_input": False,  # Enable voice input (requires microphone)
    "enable_voice_output": True,  # Enable text-to-speech output
    "enable_agent_collaboration": False,  # Enable collaboration between multiple agents
}
...
memory = {
    ...
    "browser_state": None,  # Current browser state
    ...
    "api_calls": {},  # External API call results

    # System state
    "clipboard": None,  # Clipboard content
    ...
    "image_analysis": {},  # Results of image analysis
    "data_visualizations": {},  # Generated data visualizations
    ...
    "agent_collaborators": {},  # Other agents for collaboration
    "external_tools": {},  # External tools and integrations
    "api_keys": {},  # API keys for external services (encrypted)
    ...
    # Database and voice features
    "database_queries": {},  # Database query results
    "voice_recordings": {}  # Voice recordings and transcriptions
}
...
def run_with_retry(func, *args, max_retries=None, retry_delay=None, **kwargs):
    ...
    while retries <= max_retries:
        try:
            if retries > 0:
                print(f"{Fore.YELLOW}Retry attempt {retries}/{max_retries}...{Style.RESET_ALL}")
            return func(*args, **kwargs)
        except Exception as e:
            last_exception = e
            retries += 1
            if retries <= max_retries:
                print(f"{Fore.YELLOW}Error: {str(e)}. Retrying in {retry_delay} seconds...{Style.RESET_ALL}")
                time.sleep(retry_delay)
            else:
                print(f"{Fore.RED}Maximum retries ({max_retries}) reached. Giving up.{Style.RESET_ALL}")
                break
...
def get_system_info():
    ...
    info = {
        ...
        "memory": None,  # Will be filled if psutil is available
        "disk": None,  # Will be filled if psutil is available
    }
...
def encrypt_sensitive_data(data, key=None):
    ...
    if key is None:
        # Use a derived key from machine-specific information
        # This is not secure for truly sensitive data but provides basic obfuscation
        machine_id = f"{platform.node()}:{os.getuid() if hasattr(os, 'getuid') else os.getlogin()}"
    ...
    try:
        ...
        # Fall back to basic encoding if cryptography is not available
        import base64
...
def decrypt_sensitive_data(encrypted_data, key=None):
    ...
    if key is None:
        # Use the same derived key as in encrypt_sensitive_data
        machine_id = f"{platform.node()}:{os.getuid() if hasattr(os, 'getuid') else os.getlogin()}"
        key = hashlib.sha256(machine_id.encode()).digest()
    ...
    try:
        ...
        # Fall back to basic decoding if cryptography is not available
        import base64
...
def compress_data(data):
    ...
    try:
        import gzip
        import pickle
        return base64.b64encode(gzip.compress(pickle.dumps(data))).decode()
    except Exception as e:
        print(f"{Fore.YELLOW}Warning: Failed to compress data: {e}. Using uncompressed data.{Style.RESET_ALL}")
        return data
...
def decompress_data(compressed_data):
    ...
    try:
        import gzip
        import pickle
        return pickle.loads(gzip.decompress(base64.b64decode(compressed_data.encode())))
    except Exception as e:
        print(f"{Fore.YELLOW}Warning: Failed to decompress data: {e}. Using data as-is.{Style.RESET_ALL}")
        return compressed_data
...
def send_notification(title, message):
    ...
    try:
        ...
        print(f"{Fore.YELLOW}Warning: Failed to send notification: {e}{Style.RESET_ALL}")

def validate_action_parameters(action_name, params):
    """Validate parameters for an action"""
    if not CONFIG.get("enable_action_validation", True):
        return True, ""
    ...
    required_params = {
        ...
        "BROWSE_WEB": ["url"],
        "SCREENSHOT_WEB": [],  # selector is optional
        "SEARCH_WEB": ["query"],
        "LIST_FILES": [],  # directory and pattern are optional
        "FIND_IN_FILES": ["pattern"],  # directory and file_pattern are optional
        "EXECUTE_PYTHON": ["code"],
        "EXECUTE_JAVASCRIPT": ["code"],
        "DIFF_FILES": ["file1", "file2"],
        "COPY_TO_CLIPBOARD": ["text"],
        "GET_CLIPBOARD": [],  # No required parameters
        "DOWNLOAD_FILE": ["url", "filename"],
        "SUMMARIZE_TEXT": ["text"],
        # Add validation for new actions here
    }
    ...
    for param in required_params[action_name]:
        if param not in params:
            return False, f"Missing required parameter '{param}' for {action_name}"

    # Additional validation for specific actions
    if action_name == "BROWSE_WEB" and not params["url"].startswith(("http://", "https://")):
        return False, f"Invalid URL: {params['url']}. URL must start with http:// or https://"

    if action_name == "DOWNLOAD_FILE" and not params["url"].startswith(("http://", "https://")):
        return False, f"Invalid URL: {params['url']}. URL must start with http:// or https://"
...
class AgentCLI:
    ...
    def _build_system_prompt(self):
        ...
        if CONFIG.get("enable_image_analysis", False):
            actions_list.append("18. ANALYZE_IMAGE(image_path: str) - Analyzes an image using the vision model. Example: ANALYZE_IMAGE(image_path=\"image.jpg\")")

        if CONFIG.get("enable_data_visualization", False):
            actions_list.append("19. VISUALIZE_DATA(data: str, chart_type: str = 'auto', title: str = None, filename: str = None) - Creates a visualization from data. Example: VISUALIZE_DATA(data=\"[{\\\"x\\\": 1, \\\"y\\\": 10}, {\\\"x\\\": 2, \\\"y\\\": 20}]\", chart_type=\"bar\", title=\"My Chart\")")

        if CONFIG.get("enable_git_operations", False):
            actions_list.append("20. GIT_CLONE(repo_url: str, target_dir: str = None) - Clones a Git repository. Example: GIT_CLONE(repo_url=\"https://github.com/user/repo.git\", target_dir=\"local_repo\")")
            actions_list.append("21. GIT_STATUS(repo_dir: str = '.') - Gets the status of a Git repository. Example: GIT_STATUS(repo_dir=\"my_project\")")
            actions_list.append("22. GIT_COMMIT(message: str, repo_dir: str = '.') - Commits changes to a Git repository. Example: GIT_COMMIT(message=\"Add new feature\", repo_dir=\"my_project\")")

        if CONFIG.get("enable_database_operations", False):
            actions_list.append("23. QUERY_DATABASE(connection_string: str, query: str, params: dict = None) - Executes a SQL query against a database. Example: QUERY_DATABASE(connection_string=\"sqlite:///database.db\", query=\"SELECT * FROM users WHERE age > :age\", params={\"age\": 18})")

        if CONFIG.get("enable_voice_output", False):
            actions_list.append("24. TEXT_TO_SPEECH(text: str, voice: str = 'default', output_file: str = None) - Converts text to speech. Example: TEXT_TO_SPEECH(text=\"Hello, world!\", output_file=\"greeting.mp3\")")

        if CONFIG.get("enable_voice_input", False):
            actions_list.append("25. SPEECH_TO_TEXT(audio_file: str = None, duration: int = 5) - Converts speech to text from a file or microphone. Example: SPEECH_TO_TEXT(duration=10) or SPEECH_TO_TEXT(audio_file=\"recording.wav\")")
            ...
            if response.status_code == 200:
                ...
                if isinstance(loaded_memory, dict) and "conversation" in loaded_memory:
                    ...
                    memory = {
                        ...
                        "image_analysis": loaded_memory.get("image_analysis", {}),
                        "data_visualizations": loaded_memory.get("data_visualizations", {}),
                        "git_state": loaded_memory.get("git_state", {}),
                        "database_queries": loaded_memory.get("database_queries", {}),
                        "voice_recordings": loaded_memory.get("voice_recordings", {})
                    }
  ...
  Confirm Risky Actions: {CONFIG['confirm_risky_actions']}
    ...
    def execute_parsed_action(self, action_str: str) -> ActionResult:
        ...
        try:
            ...
            elif action_name == 'ANALYZE_IMAGE' and CONFIG.get("enable_image_analysis", True):
                image_path = parsed_params.get('image_path')
                if not image_path: return ActionResult(status='error', error=f"Missing 'image_path' parameter for {action_name}")
                return self.analyze_image(image_path)

            elif action_name == 'VISUALIZE_DATA' and CONFIG.get("enable_data_visualization", True):
                data = parsed_params.get('data')
                if not data: return ActionResult(status='error', error=f"Missing 'data' parameter for {action_name}")
                chart_type = parsed_params.get('chart_type', 'auto')
                title = parsed_params.get('title')
                filename = parsed_params.get('filename')
                return self.visualize_data(data, chart_type, title, filename)

            elif action_name == 'GIT_CLONE' and CONFIG.get("enable_git_operations", True):
                repo_url = parsed_params.get('repo_url')
                if not repo_url: return ActionResult(status='error', error=f"Missing 'repo_url' parameter for {action_name}")
                target_dir = parsed_params.get('target_dir')
                return self.git_clone(repo_url, target_dir)

            elif action_name == 'GIT_STATUS' and CONFIG.get("enable_git_operations", True):
                repo_dir = parsed_params.get('repo_dir', '.')
                return self.git_status(repo_dir)

            elif action_name == 'GIT_COMMIT' and CONFIG.get("enable_git_operations", True):
                message = parsed_params.get('message')
                if not message: return ActionResult(status='error', error=f"Missing 'message' parameter for {action_name}")
                repo_dir = parsed_params.get('repo_dir', '.')
                return self.git_commit(message, repo_dir)

            elif action_name == 'QUERY_DATABASE' and CONFIG.get("enable_database_operations", False):
                connection_string = parsed_params.get('connection_string')
                if not connection_string: return ActionResult(status='error', error=f"Missing 'connection_string' parameter for {action_name}")
                query = parsed_params.get('query')
                if not query: return ActionResult(status='error', error=f"Missing 'query' parameter for {action_name}")
                params = parsed_params.get('params')
                return self.query_database(connection_string, query, params)

            elif action_name == 'TEXT_TO_SPEECH' and CONFIG.get("enable_voice_output", False):
                text = parsed_params.get('text')
                if not text: return ActionResult(status='error', error=f"Missing 'text' parameter for {action_name}")
                voice = parsed_params.get('voice', 'default')
                output_file = parsed_params.get('output_file')
                return self.text_to_speech(text, voice, output_file)

            elif action_name == 'SPEECH_TO_TEXT' and CONFIG.get("enable_voice_input", False):
                audio_file = parsed_params.get('audio_file')
                duration = parsed_params.get('duration', 5)
                return self.speech_to_text(audio_file, duration)
    ...
    def analyze_image(self, image_path: str) -> ActionResult:
        """Analyzes an image using the vision model."""
        if not CONFIG.get("enable_image_analysis", True):
            return ActionResult(status='error', error="Image analysis is disabled in configuration.")

        print(f"{Fore.CYAN}[VISION] Analyzing image: {image_path}{Style.RESET_ALL}")
        try:
            # Resolve the file path safely
            ...
            if not os.path.exists(image_path):
                return ActionResult(status='error', error=f"Image file '{relative_path}' does not exist.")
            if not os.path.isfile(image_path):
                return ActionResult(status='error', error=f"'{relative_path}' is not a file.")

            # Check file size
            file_size = os.path.getsize(image_path)
            if file_size > CONFIG.get("max_file_size_read", 5 * 1024 * 1024):
                return ActionResult(status='error', error=f"Image file is too large ({format_size(file_size)}). Maximum allowed size is {format_size(CONFIG.get('max_file_size_read', 5 * 1024 * 1024))}.")
            ...
            try:
                ...
                return ActionResult(status='error', error=f"Failed to open image file: {img_err}")
            ...
            # Create a model instance
            model = genai.GenerativeModel(model_name=CONFIG.get("vision_model", "gemini-2.5-pro-vision"))

            # Generate the analysis
            img_data = pathlib.Path(image_path)
            prompt = "Analyze this image in detail. Describe what you see, including objects, people, text, colors, and any other notable elements."
            ...
            image_id = hashlib.md5(relative_path.encode()).hexdigest()[:8]
            memory["image_analysis"][image_id] = {
                "timestamp": time.time(),
                "path": relative_path,
                "analysis": analysis,
                "metadata": {
            ...
            # Format output
            output_text = f"Image Analysis for '{relative_path}':\n\n{analysis}"
            print(f"{Fore.GREEN}Image successfully analyzed.{Style.RESET_ALL}")
            return ActionResult(status='success', output=output_text, data={
                "analysis": analysis,
                "image_id": image_id,
                "metadata": memory["image_analysis"][image_id]["metadata"]
            })
    ...
    def visualize_data(self, data: str, chart_type: str = 'auto', title: str = None, filename: str = None) -> ActionResult:
        """Creates a visualization from data."""
        if not CONFIG.get("enable_data_visualization", True):
            return ActionResult(status='error', error="Data visualization is disabled in configuration.")

        if not MATPLOTLIB_AVAILABLE:
            return ActionResult(status='error', error="Matplotlib is required for data visualization but is not installed.")

        print(f"{Fore.CYAN}[VISUALIZE] Creating {chart_type} visualization{Style.RESET_ALL}")
        try:
            # Parse the data
            try:
                ...
                try:
                    ...
                    import io
                    import pandas as pd
                    parsed_data = pd.read_csv(io.StringIO(data))
                except Exception:
                    # Try to parse as a Python literal (list/dict)
                    import ast
                    try:
                        parsed_data = ast.literal_eval(data)
                    except Exception:
                        return ActionResult(status='error', error="Could not parse data. Please provide data in JSON, CSV, or Python literal format.")
            ...
            if isinstance(parsed_data, pd.DataFrame):
                ...
                if all(isinstance(v, dict) for v in parsed_data.values()):
                ...
                return ActionResult(status='error', error=f"Unsupported data format: {type(parsed_data)}")
            ...
            if chart_type == 'auto':
                ...
                if num_columns == 0:
                    chart_type = 'table'  # No numeric columns
                elif num_columns == 1:
                    chart_type = 'bar'  # Single numeric column
                elif num_columns == 2:
                    chart_type = 'scatter'  # Two numeric columns
                elif num_rows <= 10 and num_columns <= 5:
                    chart_type = 'bar'  # Small dataset
                else:
                    chart_type = 'heatmap'  # Large dataset
            ...
            if chart_type == 'bar':
                ...
                if len(numeric_cols) == 0:
                    return ActionResult(status='error', error="No numeric columns found for bar chart.")

                if len(df) > 20:  # Too many rows for a readable bar chart
                    df = df.head(20)  # Limit to first 20 rows

                ax = df.plot(kind='bar', y=numeric_cols[0] if len(numeric_cols) > 0 else None)
                plt.xticks(rotation=45)

            elif chart_type == 'line':
                numeric_cols = df.select_dtypes(include=['number']).columns
                if len(numeric_cols) == 0:
                    return ActionResult(status='error', error="No numeric columns found for line chart.")

                ax = df.plot(kind='line', y=numeric_cols)

            elif chart_type == 'scatter':
                numeric_cols = df.select_dtypes(include=['number']).columns
                if len(numeric_cols) < 2:
                    return ActionResult(status='error', error="Need at least two numeric columns for scatter plot.")

                ax = df.plot(kind='scatter', x=numeric_cols[0], y=numeric_cols[1])

            elif chart_type == 'pie':
                numeric_cols = df.select_dtypes(include=['number']).columns
                if len(numeric_cols) == 0:
                    return ActionResult(status='error', error="No numeric columns found for pie chart.")

                ax = df.plot(kind='pie', y=numeric_cols[0], autopct='%1.1f%%')

            elif chart_type == 'histogram':
                numeric_cols = df.select_dtypes(include=['number']).columns
                if len(numeric_cols) == 0:
                    return ActionResult(status='error', error="No numeric columns found for histogram.")

                ax = df.hist(column=numeric_cols[0], bins=10)

            elif chart_type == 'heatmap':
                # For heatmap, we need numeric data
                numeric_df = df.select_dtypes(include=['number'])
                if numeric_df.empty:
                    return ActionResult(status='error', error="No numeric columns found for heatmap.")
                ...
                numeric_cols = df.select_dtypes(include=['number']).columns
                if len(numeric_cols) == 0:
                    return ActionResult(status='error', error="No numeric columns found for box plot.")

                ax = df.boxplot(column=numeric_cols)
                ...
                # Display as table
                table_data = df.head(10)  # Limit to first 10 rows
                table = plt.table(
                ...
                return ActionResult(status='error', error=f"Unsupported chart type: {chart_type}")
            ...
            # Store in memory
            viz_id = hashlib.md5(relative_path.encode()).hexdigest()[:8]
            memory["data_visualizations"][viz_id] = {
                "timestamp": time.time(),
                "path": relative_path,
            ...
            # Format output
            output_text = f"{chart_type.capitalize()} chart created and saved to '{relative_path}'."
            print(f"{Fore.GREEN}{output_text}{Style.RESET_ALL}")
            return ActionResult(status='success', output=output_text, data={
                "filename": relative_path,
                "chart_type": chart_type,
                "visualization_id": viz_id,
                "data_shape": memory["data_visualizations"][viz_id]["data_shape"]
            })
    ...
    def git_clone(self, repo_url: str, target_dir: str = None) -> ActionResult:
        """Clones a Git repository."""
        if not CONFIG.get("enable_git_operations", True):
            return ActionResult(status='error', error="Git operations are disabled in configuration.")

        print(f"{Fore.CYAN}[GIT] Cloning repository: {repo_url}{Style.RESET_ALL}")
        try:
            # Check if git is installed
            try:
                subprocess.run(["git", "--version"], check=True, capture_output=True)
            except (subprocess.SubprocessError, FileNotFoundError):
                return ActionResult(status='error', error="Git is not installed or not in PATH.")

            # Prepare the command
            cmd = ["git", "clone", repo_url]
            if target_dir:
                # Resolve the target directory path safely
                target_dir = self._resolve_path(target_dir)
                relative_dir = self._get_relative_path(target_dir)
                cmd.append(target_dir)
            else:
                # Extract repo name from URL for relative path reporting
                repo_name = repo_url.split('/')[-1]
                if repo_name.endswith('.git'):
                    repo_name = repo_name[:-4]
                relative_dir = repo_name

            # Execute the command
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=CONFIG.get("command_timeout", 120))

            if result.returncode == 0:
                # Store git state in memory
                repo_id = hashlib.md5(repo_url.encode()).hexdigest()[:8]
                memory["git_state"][repo_id] = {
                    "timestamp": time.time(),
                    "repo_url": repo_url,
                ...
                # Format output
                output_text = f"Repository cloned successfully to '{relative_dir}'."
                print(f"{Fore.GREEN}{output_text}{Style.RESET_ALL}")
                return ActionResult(status='success', output=output_text, data={
                    "repo_url": repo_url,
                    "local_path": relative_dir,
            ...
            error_msg = f"Git clone operation timed out after {CONFIG.get('command_timeout', 120)} seconds."
            print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
            return ActionResult(status='error', error=error_msg)
        except Exception as e:
    ...
    def git_status(self, repo_dir: str = '.') -> ActionResult:
        """Gets the status of a Git repository."""
        if not CONFIG.get("enable_git_operations", True):
            return ActionResult(status='error', error="Git operations are disabled in configuration.")

        print(f"{Fore.CYAN}[GIT] Getting status of repository in '{repo_dir}'{Style.RESET_ALL}")
        try:
            # Check if git is installed
            try:
                subprocess.run(["git", "--version"], check=True, capture_output=True)
            ...
            # Resolve the directory path safely
            repo_dir = self._resolve_path(repo_dir)
            relative_dir = self._get_relative_path(repo_dir)
            ...
            if not os.path.exists(git_dir) or not os.path.isdir(git_dir):
                return ActionResult(status='error', error=f"'{relative_dir}' is not a Git repository.")

            # Get repository status
            result = subprocess.run(["git", "status"], cwd=repo_dir, capture_output=True, text=True, timeout=CONFIG.get("command_timeout", 120))
            ...
            if result.returncode == 0:
                ...
                commit_result = subprocess.run(["git", "log", "-1", "--pretty=format:%h %s (%an, %ar)"], cwd=repo_dir, capture_output=True, text=True)
                last_commit = commit_result.stdout.strip() if commit_result.returncode == 0 else "No commits yet"

                # Store git state in memory
                repo_id = hashlib.md5(relative_dir.encode()).hexdigest()[:8]
                memory["git_state"][repo_id] = {
                    "timestamp": time.time(),
                    "local_path": relative_dir,
                ...
                # Format output
                output_text = f"Git Status for '{relative_dir}':\n\nCurrent Branch: {current_branch}\nLast Commit: {last_commit}\n\n{result.stdout}"
                print(f"{Fore.GREEN}Git status retrieved successfully.{Style.RESET_ALL}")
                return ActionResult(status='success', output=output_text, data={
                ...
                error_msg = f"Git status failed: {result.stderr.strip()}"
                print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
                return ActionResult(status='error', error=error_msg)

        except subprocess.TimeoutExpired:
            error_msg = f"Git status operation timed out after {CONFIG.get('command_timeout', 120)} seconds."
            print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
            return ActionResult(status='error', error=error_msg)
        except Exception as e:
            error_msg = f"Error getting repository status: {e}"
            print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
            print(f"{Fore.RED}{traceback.format_exc()}{Style.RESET_ALL}")
            return ActionResult(status='error', error=error_msg)

    def git_commit(self, message: str, repo_dir: str = '.') -> ActionResult:
        """Commits changes to a Git repository."""
        if not CONFIG.get("enable_git_operations", True):
            return ActionResult(status='error', error="Git operations are disabled in configuration.")

        print(f"{Fore.CYAN}[GIT] Committing changes in '{repo_dir}' with message: {message}{Style.RESET_ALL}")
        try:
            # Check if git is installed
            try:
                subprocess.run(["git", "--version"], check=True, capture_output=True)
            ...
            if not status_result.stdout.strip():
                return ActionResult(status='error', error="No changes to commit.")

            # Add all changes
            add_result = subprocess.run(["git", "add", "."], cwd=repo_dir, capture_output=True, text=True)
            if add_result.returncode != 0:
                error_msg = f"Failed to add changes: {add_result.stderr.strip()}"
                print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
                return ActionResult(status='error', error=error_msg)
            ...
            if commit_result.returncode == 0:
                ...
                # Get commit information
                commit_hash_result = subprocess.run(["git", "rev-parse", "HEAD"], cwd=repo_dir, capture_output=True, text=True)
                ...
                memory["git_state"][repo_id] = {
                    ...
                    "last_operation": "commit"
                }

                # Format output
                output_text = f"Changes committed successfully to '{relative_dir}':\n\n{commit_result.stdout}"
                print(f"{Fore.GREEN}{output_text}{Style.RESET_ALL}")
                return ActionResult(status='success', output=output_text, data={
                    "repo_dir": relative_dir,
                ...
                error_msg = f"Git commit failed: {commit_result.stderr.strip()}"
                print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
                return ActionResult(status='error', error=error_msg)

        except subprocess.TimeoutExpired:
            error_msg = f"Git commit operation timed out after {CONFIG.get('command_timeout', 120)} seconds."
            print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
            return ActionResult(status='error', error=error_msg)
        except Exception as e:
            error_msg = f"Error committing changes: {e}"
            print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
            print(f"{Fore.RED}{traceback.format_exc()}{Style.RESET_ALL}")
            return ActionResult(status='error', error=error_msg)

    def query_database(self, connection_string: str, query: str, params: dict = None) -> ActionResult:
        """Executes a SQL query against a database."""
        if not CONFIG.get("enable_database_operations", False):
            return ActionResult(status='error', error="Database operations are disabled in configuration.")

        print(f"{Fore.CYAN}[DATABASE] Executing query against database{Style.RESET_ALL}")
        try:
            ...
            try:
                ...
                print(f"{Fore.YELLOW}SQLAlchemy not found. Installing...{Style.RESET_ALL}")
            ...
            with engine.connect() as connection:
                ...
                if query.strip().lower().startswith("select"):
                    ...
                    memory["database_queries"] = memory.get("database_queries", {})
                    memory["database_queries"][query_id] = {
                        "timestamp": time.time(),
                        "query": query,
                    ...
                    if len(results) > 0:
                        ...
                        col_widths = {col: max(len(str(col)), max([len(str(row[col])) for row in results])) for col in columns}
                        header = " | ".join([str(col).ljust(col_widths[col]) for col in columns])
                        separator = "-+-".join(["-" * col_widths[col] for col in columns])
                        rows_formatted = [" | ".join([str(row[col]).ljust(col_widths[col]) for col in columns]) for row in results[:10]]

                        table = f"{header}\n{separator}\n" + "\n".join(rows_formatted)
                        if len(results) > 10:
                            table += f"\n... and {len(results) - 10} more rows"

                        output_text = f"Query executed successfully. Returned {len(results)} rows.\n\n{table}"
                    else:
                    ...
                    return ActionResult(status='success', output=output_text, data={
                        "query_id": query_id,
                        "row_count": len(results),
                        "results": results[:100],  # Limit to first 100 rows to avoid huge responses
                        "columns": list(columns),
                    ...
                    output_text = f"Query executed successfully. Affected {row_count} rows."
                    print(f"{Fore.GREEN}{output_text}{Style.RESET_ALL}")
                    return ActionResult(status='success', output=output_text, data={
                        "row_count": row_count
                    })
            ...
            error_msg = f"Error executing database query: {e}"
            print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
            print(f"{Fore.RED}{traceback.format_exc()}{Style.RESET_ALL}")
            return ActionResult(status='error', error=error_msg)

    def text_to_speech(self, text: str, voice: str = "default", output_file: str = None) -> ActionResult:
        """Converts text to speech."""
        if not CONFIG.get("enable_voice_output", False):
            return ActionResult(status='error', error="Voice output is disabled in configuration.")

        print(f"{Fore.CYAN}[SPEECH] Converting text to speech{Style.RESET_ALL}")
        try:
            # Check if required packages are installed
            try:
                import gtts
                ...
                print(f"{Fore.YELLOW}gTTS not found. Installing...{Style.RESET_ALL}")
                subprocess.check_call([sys.executable, "-m", "pip", "install", "gtts"])
                from gtts import gTTS

            # Generate a default output filename if none provided
            if not output_file:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                output_file = f"speech_{timestamp}.mp3"

            # Resolve the file path safely
            file_path = self._resolve_path(output_file)
            relative_path = self._get_relative_path(file_path)
            ...
            try:
                ...
                playback_status = "Audio playback started."
            except Exception as play_err:
                playback_status = f"Audio saved but could not be played automatically: {play_err}"

            # Format output
            output_text = f"Text converted to speech and saved to '{relative_path}'. {playback_status}"
            print(f"{Fore.GREEN}Text successfully converted to speech.{Style.RESET_ALL}")
            return ActionResult(status='success', output=output_text, data={
                "filename": relative_path,
                "file_size": file_size,
    ...
    def speech_to_text(self, audio_file: str = None, duration: int = 5) -> ActionResult:
        """Converts speech to text from a file or microphone."""
        if not CONFIG.get("enable_voice_input", False):
            return ActionResult(status='error', error="Voice input is disabled in configuration.")

        print(f"{Fore.CYAN}[SPEECH] Converting speech to text{Style.RESET_ALL}")
        try:
            # Check if required packages are installed
            try:
                import speech_recognition as sr
            except ImportError:
                print(f"{Fore.YELLOW}SpeechRecognition not found. Installing...{Style.RESET_ALL}")
            ...
            # Process from file or microphone
            if audio_file:
                # Resolve the file path safely
                audio_path = self._resolve_path(audio_file)
                relative_path = self._get_relative_path(audio_path)

                # Check if file exists
                if not os.path.exists(audio_path):
                    return ActionResult(status='error', error=f"Audio file '{relative_path}' does not exist.")
                ...
                with sr.AudioFile(audio_path) as source:
                    ...
                    source_info = f"audio file '{relative_path}'"
            else:
                # Record from microphone
                print(f"{Fore.YELLOW}Listening for {duration} seconds...{Style.RESET_ALL}")
                with sr.Microphone() as source:
                    recognizer.adjust_for_ambient_noise(source)
                    audio_data = recognizer.listen(source, timeout=duration)
                    source_info = f"microphone (duration: {duration}s)"

            # Recognize speech
            text = recognizer.recognize_google(audio_data)

            # Format output
            output_text = f"Speech from {source_info} converted to text:\n\n{text}"
            print(f"{Fore.GREEN}Speech successfully converted to text.{Style.RESET_ALL}")
            return ActionResult(status='success', output=output_text, data={
                "text": text,
                "source": "file" if audio_file else "microphone",
                "source_path": relative_path if audio_file else None,
                "duration": duration if not audio_file else None
            })

        except Exception as e:
            error_msg = f"Error converting speech to text: {e}"
            print(f"{Fore.RED}{error_msg}{Style.RESET_ALL}")
            print(f"{Fore.RED}{traceback.format_exc()}{Style.RESET_ALL}")
            return ActionResult(status='error', error=error_msg)
